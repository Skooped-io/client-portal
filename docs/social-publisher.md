...
select files on /m  →  Queue for posting (FB / IG / both)  →  DRAFT rows
                                                              ...
                                                              Approve
                     ┌───────────────────────────────────────────┴──────────────────────────────┐
   Facebook, 10 min – 30 days out                                      Instagram (any future time)
   Meta SCHEDULED post created now                                     row status 'approved'
   (published=false + scheduled_publish_time)                          /api/cron/social-publish (every 5 min)
   ...
   Under 10 minutes / past: published immediately (either platform).   Facebook beyond 30 days: cron too.
```

Row states (`social_posts.status`): `draft → approved → scheduled | publishing → published`,
with `failed` (last_error shown on the page; Approve = retry) and `cancelled`.
...
`publishing` or `published` no user action applies.

Files used in a published post get `capture_uploads.posted_at` + `post_ref`
(`fb:<post id>` / `ig:<media id>`), the same columns "Mark posted" writes.
...
| `CRON_SECRET` | exists | Bearer auth on `/api/cron/social-publish` (same as the other crons) |
...
ratio (4:5 … 1.91:1), width capped at 1440, quality 85, stored at
...
- FB `/photos`: `url`, `caption`, `published=false`, `scheduled_publish_time`
  ...
  `attached_media[n]={"media_fbid":…}`. Scheduling window is 10 minutes to
  30 days (the `/feed` reference says 75 days; real (#100) errors say 30).
  We read the post back and abort on a scheduled-time mismatch.
...
- The cron does not yet retry a `publishing` row that died mid-flight — it
  stays visible as "Publishing…" for a manual look (same posture as GBP posts).
