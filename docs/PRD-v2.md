...
> **Historical note (2026-08-04):** this PRD was originally written around a roster of
> named AI agent personas. That roster was retired on 2026-08-04. There is one identity
> in the product now: Skooped. Sections that described per persona profiles, avatars and
> bylines have been rewritten or marked retired below.
...
Ice cream brand identity (medium intensity). Dark mode. Framer-motion animations. Full workflow integration with Axiom logging, GitHub tracking, and Slack communications.
...
**Layout:**
- Fixed left sidebar (collapsible) with nav
...
- Timeline of what Skooped did for this client
- Skooped logo next to each action ("Keywords updated", "3 posts scheduled")
- Animated entry, items slide in from right
- "View all" link to full activity log

**AI Insights Panel:**
- Skooped's latest recommendations
- Styled as a chat-like interface with the Skooped logo
- "Your top keyword dropped 3 spots, heres what we recommend"
- Action buttons on recommendations ("Apply", "Dismiss", "Ask Skooped")
...
- Automated report badge ("Generated by Skooped")

### 8. Team Page (RETIRED 2026-08-04)
This page existed only to display the named AI persona roster (cards, avatars, per persona
bios and activity). The roster was retired on 2026-08-04 and the page was removed with it.
Recent work is now surfaced through the activity feed on the dashboard, attributed to
Skooped rather than to individual names. Section number kept so the rest of the doc still
lines up.
...
- Quick actions: "Go to SEO", "View latest report", "Ask Skooped"
- Recent pages
- Keyboard navigation
...
### Activity Display
- Dashboard pulls recent activity from Supabase
- Shows real actions taken on the account (not fake/simulated)
- Attributed to Skooped, not to individual names
...
| Activity | Supabase agent_activity table | Real-time |
...
### Must Have (v2 Launch)
- [ ] All pages built and functional (section 8 retired, so 9 remain)
...
10. Every PR is reviewed before merge
