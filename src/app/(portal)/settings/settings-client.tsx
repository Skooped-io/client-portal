...
const DEMO_PROFILE: SettingsProfile = {
  name: 'Your Name',
...
const DEMO_TEAM: SettingsTeamMember[] = [
  { id: '1', name: 'Account Owner', email: 'owner@yourbusiness.com', role: 'admin', avatar: '' },
  { id: '2', name: 'Team Member', email: 'teammate@yourbusiness.com', role: 'viewer', avatar: '' },
]

const DEMO_PLAN: SettingsPlan = {
  name: 'Growth',
  price: '$129',
  interval: 'month',
...
function AccountTab({ profile }: { profile: SettingsProfile }) {
        ...
        <Card className="bg-card border-border rounded-2xl">
          ...
          <CardContent className="divide-y divide-border">
            ...
            <ToggleSwitch
              ...
              description="Full report generated on the 1st of each month"
            />
            <ToggleSwitch
...
function BillingTab({ plan }: { plan: SettingsPlan }) {
  return (
    <motion.div variants={stagger} initial="hidden" animate="visible" className="space-y-6">
      {/* Current Plan */}
      <motion.div variants={slideUp}>
          ...
          <div className="bg-gradient-to-r from-strawberry/10 to-mint/10 px-6 py-5 border-b border-border">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-nunito font-bold text-foreground text-lg">{plan.name} Plan</p>
                  <p className="text-xs text-muted-foreground">Active since January 2026</p>
                </div>
              </div>
              <div className="text-right">
                <p className="text-2xl font-nunito font-bold text-foreground">{plan.price}</p>
          ...
          <CardContent className="p-6">
            ...
            <div className="mb-4">
              ...
              <div className="w-full h-2 bg-muted rounded-full overflow-hidden">
                <motion.div
                  initial={{ width: 0 }}
                  animate={{ width: `${plan.usagePercent}%` }}
          ...
          <CardContent>
            <div className="flex items-center justify-between p-4 rounded-xl border border-border bg-background">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-medium text-foreground">{plan.card}</p>
                  <p className="text-xs text-muted-foreground">Expires {plan.cardExpiry}</p>
                </div>
              </div>
...
function TeamTab({ members: initialMembers }: { members: SettingsTeamMember[] }) {
...
export default function SettingsPage({ data }: { data?: SettingsPageData } = {}) {
  const [activeTab, setActiveTab] = useState<TabId>('account')
  const [isLoading] = useState(false)

  const profile = data?.profile ?? DEMO_PROFILE
  const team = data?.team ?? DEMO_TEAM
  const plan = data?.plan ?? DEMO_PLAN

  const tabContent: Record<TabId, React.ReactNode> = {
    account: <AccountTab profile={profile} />,
    connections: <ConnectionsTab />,
    notifications: <NotificationsTab />,
    billing: <BillingTab plan={plan} />,
    team: <TeamTab members={team} />,
  }
